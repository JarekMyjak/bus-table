...
for (const line of linesToScrape) {
    for (const direction of [0, 1]) {
        ...
        const stopNames = await page.evaluate(() => {
            return [...document.querySelectorAll('.bs a')].map((element) => { var _a; return (_a = element.textContent) !== null && _a !== void 0 ? _a : (element.textContent = ''); });
        });
        for (const [stopIndex, stopName] of stopNames.entries()) {
            ...
            const stopTimes = await page.evaluate(() => {
                ...
                for (const row of rowElements) {
                    const hour = (_a = row.querySelector("td.h")) === null || _a === void 0 ? void 0 : _a.textContent;
                    const minutes = [...row.querySelectorAll("td.m")].map((min) => min === null || min === void 0 ? void 0 : min.textContent);
